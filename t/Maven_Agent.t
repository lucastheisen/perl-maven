...
SKIP: {
    ...
    my $agent = Maven::Agent->new(
        M2_HOME => File::Spec->catdir($test_dir, 'M2_HOME'),
        'user.home' => $userHome);
    $expected_file = File::Spec->catfile($test_dir, 'WIN_HOME', '.m2', 'repository', 
        'com', 'pastdev', 'foo', '1.0.2', 'foo-1.0.2.pom');
    $to_file = File::Temp->new();
    my $file = $agent->download('com.pastdev:foo:pom:1.0.2',
        to => $to_file);
    ...
    my $to_dir = File::Temp->newdir();
    $file = $agent->download('com.pastdev:foo:pom:1.0.2',
        to => $to_dir);
    my $expected_download_file = File::Spec->catfile($to_dir, 'foo.pom');
